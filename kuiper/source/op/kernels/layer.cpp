...
base::Status Layer::check_tensor_with_dim(const tensor::Tensor& tensor, base::DeviceType device_type, base::DataType data_type,
                                         ...)const{
    if (tensor.is_empty()) {
        return base::error::InvalidArgument("The tensor parameter is empty.");
    }
    ...
    //va_start 是C/C++中用于处理可变参数函数的宏，需配合 va_arg 和 va_end 使用。
    //使用 va_start 初始化参数列表指针，使其指向最后一个固定参数：
    std::va_list args;
    va_start(args, data_type);
    int32_t dim = tensor.dims_size();
    for(int i = 0;i < dim;i++){
        //通过 va_arg 逐个读取参数，需指定参数类型：
...
//以上大部分涉及tensor的操作都是封装了Armando的api
